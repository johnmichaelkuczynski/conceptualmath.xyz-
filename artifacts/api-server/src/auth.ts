...
export function setupAuth(app: Express) {
  ...
  app.use(
    session({
      store: pgStore,
      secret: process.env.SESSION_SECRET || "developmental-mathematics-secret-key",
      resave: false,
  ...
  if (googleEnabled) {
    ...
    // registered in the owner's Google Cloud Console OAuth client. (This app's
    // shared proxy routes only /api paths to the API server.)
